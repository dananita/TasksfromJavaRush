...
public class Field {
    ...
    public void removeFullLines() {
        ...
        List<int[]> linesFormatrix = new ArrayList<>();
        ...
        for (int y = 0; y < height; y++) {
            ...
            if (count != width){
                linesFormatrix.add(matrix[y]);
            }
        }

        //Добавляем недостающие строки в начало списка.
        while (linesFormatrix.size() < height){
            linesFormatrix.add(0,new int[width]);
        }

        //Преобразуем список обратно в матрицу
        matrix = linesFormatrix.toArray(new int[height][width]);
    }
}
